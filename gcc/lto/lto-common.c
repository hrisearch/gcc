/* LTO common functions between lto.c and lto-dump.c.
...
GTY((length ("lto_stats.num_input_files + 1"))) struct lto_file_decl_data **all_file_decl_data;
...
/* Input file data before flattening (i.e. splitting them to subfiles to support
   incremental linking.  */
int real_file_count;
GTY((length ("real_file_count + 1"))) struct lto_file_decl_data **real_file_decl_data;
...
read_cgraph_and_symbols (unsigned nfiles, const char **fnames)
{
  ...
  /* hash_set to handle repeated object files.  */
  hash_set <lto_file*> set;
  ...
  for (i = 0, last_file_ix = 0; i < nfiles; ++i)
	...
	break;
      ...
      /* Read each lto file only once.  */
      if (!set.contains (current_lto_file))
      {
	set.add (current_lto_file);
